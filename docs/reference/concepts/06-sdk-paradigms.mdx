...
Though some context data may remain static for the entire application lifecycle (such as the hostname or application version) much of it may change frequently.
...
When the context data changes, it's often necessary for the provider to update it's cache of evaluated flags, or request a updated ruleset from the server; we refer to this as _context reconiliation_.
...
These are emitted accordingly when the provider's context is invalidated, and after subsequently updated.
